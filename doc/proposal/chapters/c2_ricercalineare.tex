...
\begin{itemize}
    ...
    \item Velocità del processore:  0.66 TFlops
    \item Dimensione dell'hash : 32 byte 
    \item SHA256 Ops totali: 512 Flops
\end{itemize}

La lunghezza media delle password è esagerata, ma è stata scelta in modo da coprire anche quei database su cui è stata posta una policy molto stringente in termini di lunghezza delle password. La velocità della rete è presa direttamente dal sito di hpc4ai\cite{hpc4ai} che riporta la velocità della ethernet di connessione tra i nodi, la velocità del disco fisso è stimata considerando che il cluster usi degli HDD di tipo enterprise con interfaccia SAS, che normalmente riescono in lettura a offrire un range di velocità che varia dai 250 MB/s a 1GB/s. La velocità del processore invece è stimata partendo dalla velocità base di un Intel Xeon E5-2697V4, un'architettura disponibile per i formati server, usando la seguente formula
$$
Gflops= Freq_{ghz}*Ncores*OPpercycle
$$
per ottenere i teraflops è sufficiente $TFlops=\frac{Gflops}{1000}$. Il numero di operazioni necessarie invece per calcolare un blocco da 64 byte in SHA256 con 64 round è $SHA_{ops}=64*(6+2)$ dove 6 sono le operazioni logiche AND, OR, XOR, NOT, shift, rotate a cui il blocco viene sottoposto mentre 2 sono le due addizioni modulari $\mod 2^{32}$ che vengono compiute dall'algoritmo, per cui per ogni blocco sono necessarie 512 operazioni , in questo caso intere, ma che possono essere approssimate a flops. A questo punto si può calcolare il tempo necessario per calcolare l'hash di una singola stringa, prendendo in esame l'Intel Xeon precedente si ottiene $T_{hash}=\frac{512}{0.66*10^12}=7.75*10^{-10}s$.  
...
Dal grafico si evince subito che l'algoritmo scala piuttosto bene all'aumentare dell'input e si dimostra anche piuttosto efficiente nonostante la sua semplicità. Di fatto in questa istanza non sono state considerate diverse variabili, che potrebbero modificare profondamente il risultato di questa stima, per esempio il tempo di spostamento dalla RAM alla cache del processore quando avviene un cache miss oppure il ritardo dovuto allo scheduling dei task da parte del sistema operativo, variabili che possono influenzare molto l'esecuzione e per cui un benchmark fornirebbe dati sicuramente più attendibili.


Un ultima analisi va fatta considerando lo spazio che deve essere occupato in RAM per eseguire il processo. Per quanto vi sia la necessità di condividere uno dei due insiemi (il database e le password) in copia completa su tutti gli esecutori, a livello pratico è utile pensare che non è detto che tutti i task vengano eseguiti su macchine diverse, per cui sono disponibili più strategie. Se si vuole mantenere la tecnologia di parallelizzazione omogenea, per cui si vuole usare solo MPI senza ricorrere al multithreading per la parallelizzazione, un utile metodo potrebbe essere quello di memorizzare la copia dell'insieme scelto, in questo caso quello delle password, in shared memory, così che tutti i task che vengono eseguiti sulla stessa macchina possano accedere a quel segmento senza per forza riceverne una copia da usare localmente al processo. Se invece si opta per l'utilizzo del multithread la memoria è già condivisa e quindi non è necessario ricorrere esplicitamente alla shared memory.
