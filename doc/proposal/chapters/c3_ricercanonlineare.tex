...
\begin{lstlisting}
    ...
    map = hashmap(space)
    for hash in database do{
        index = bigint_read(hash) % space 
        map.append(hash,index)
    }
...
L'architettura con cui si affronta questo problema è la solita root-worker, con la differenza che i nodi si troveranno task di tipo diverso tra una fase e l'altra, le fasi sono in tutto 3, ordinamento degli hash nel database tra i vari nodi, calcolo e distribuzione degli hash delle password tra i nodi e infine confronto tra password e hash del database. Queste 3 fasi delineano 3 tipi di task diversi, di cui i primi 2 indipendenti dall'ordine di esecuzione, mentre il terzo necessita che i primi 2 siano stati completati. Procedendo nel modo più semplice, la prima fase prevede che il file del database venga scansionato linearmente e che ad ogni hash venga applicata l'operazione di modulo, per risolvere questa prima fase è sufficiente una triviale strategia di tipo divide and conquer, per cui si divide il file del database in P parti e si procede al calcolo in modo lineare, una volta finito ogni nodo trasmette al root il risultato che a sua volta