...
__host__ size_t static inline get_data_size(uint8_t *sizes, size_t num_of_strings){
...
__global__ void md5_apply_gpu(struct md5_transform_request req){
  ...
  if(i < req.num_of_strings){
    ...
    memcpy(req.result + i*32,result,32);
...
__host__ static inline struct md5_transform_request copy_trasform_request(struct md5_transform_request * req){
  ...
  try(cudaMalloc(&devptr.sizes, req->num_of_strings));
  ...
  try(cudaMemcpy(devptr.sizes, req->sizes, req->num_of_strings, cudaMemcpyHostToDevice));
...
void md5_gpu_transform(struct md5_transform_request request, int maxthreads) {
  ...
  cudaMemcpy(request.result, devptr.result, request.num_of_strings*32, cudaMemcpyDeviceToHost);
