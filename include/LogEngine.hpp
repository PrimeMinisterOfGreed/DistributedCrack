...
{
	return *new std::string(std::vformat(std::string_view(format), std::make_format_args(args...)));
}
...
private:
	...
	std::ostream& log();
public:
	static void CreateInstance(boost::mpi::communicator& comm, std::istream* loadStream, std::ostream* saveStream, int verbosity = 1);
