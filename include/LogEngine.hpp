...
std::string makeformat(const char *format, Args... args) {
  return std::string(
      fmt::vformat(std::string(format), fmt::make_format_args(args...)));
}
...
public:
  ...
  template <typename... Args>
  void TraceDebug(const char *format, Args... args) {
...
private:
  ...
  MPILogEngine(boost::mpi::communicator &comm, std::istream *loadStream,
               std::ostream *saveStream, int verbosity = 1);
  int _verbosity = 1;
  static MPILogEngine *_instance;
  std::ostream &log();

public:
  static void CreateInstance(boost::mpi::communicator &comm,
